...
namespace graphlab {
  ...
  /**
   * by @anilpacaci (apacaci@uwaterloo.ca)
   * This engine code has been adopted from PowerLyra sync engine for edge-cut
   * partitioning. Edge-cut partitioners group edges by source vertices, which
   * means that source of each edge is the master. After gather, master 
   * accumulates the final result but it does not need to be sync with mirrors. 
   * Because in next iterations, gather will fetch data from source, which is 
   * going to be the local master. No data from mirrors will be fetched.
   * 
   * 
   * \ingroup engines
   *
  ...
  class powerlyra_sync_ec_engine :
  ...
  private:
     ...
     */
    typedef context<powerlyra_sync_ec_engine> context_type;
    friend class context<powerlyra_sync_ec_engine>;
  ...
  public:
     ...
     */
    powerlyra_sync_ec_engine(distributed_control& dc, graph_type& graph,
    ...
    void run_synchronous(MemberFunction member_fun) {
      ...
      for(size_t i = 0; i < ncpus; ++i) {
        ...
        threads.launch(boost::bind(
              &powerlyra_sync_ec_engine::thread_launch_wrapped_event_counter,
              this,
              invoke), affinity);
      }
   ...
   */
  template<typename VertexProgram>
  powerlyra_sync_ec_engine<VertexProgram>::
  powerlyra_sync_ec_engine(distributed_control& dc,
                     graph_type& graph,
  ...
  template<typename VertexProgram>
  void powerlyra_sync_ec_engine<VertexProgram>:: init() {
  ...
  template<typename VertexProgram>
  void powerlyra_sync_ec_engine<VertexProgram>:: resize() {
  ...
  template<typename VertexProgram>
  typename powerlyra_sync_ec_engine<VertexProgram>::aggregator_type*
  powerlyra_sync_ec_engine<VertexProgram>::get_aggregator() {
    return &aggregator;
  } // end of get_aggregator


  template<typename VertexProgram>
  void powerlyra_sync_ec_engine<VertexProgram>::internal_stop() {
  ...
  template<typename VertexProgram>
  void powerlyra_sync_ec_engine<VertexProgram>::rpc_stop() {
    force_abort = true;
  } // end of rpc_stop


  template<typename VertexProgram>
  void powerlyra_sync_ec_engine<VertexProgram>::
  ...
  internal_signal_gvid(vertex_id_type gvid, const message_type& message) {
    ...
    if(proc == rmi.procid()) internal_signal_rpc(gvid, message);
                         ...
                         &powerlyra_sync_ec_engine<VertexProgram>::internal_signal_rpc,
                         gvid, message);
  ...
  template<typename VertexProgram>
  size_t powerlyra_sync_ec_engine<VertexProgram>::
  num_updates() const { return completed_applys.value; }

  template<typename VertexProgram>
  float powerlyra_sync_ec_engine<VertexProgram>::
  elapsed_seconds() const { return timer::approx_time_seconds() - start_time; }

  template<typename VertexProgram>
  int powerlyra_sync_ec_engine<VertexProgram>::
  iteration() const { return iteration_counter; }



  template<typename VertexProgram>
  size_t powerlyra_sync_ec_engine<VertexProgram>::total_memory_usage() const {
  ...
  template<typename VertexProgram> 
  execution_status::status_enum powerlyra_sync_ec_engine<VertexProgram>::
  start() {
    if (vlocks.size() != graph.num_local_vertices())
      resize();
    completed_gathers = 0;
    ...
    while(iteration_counter < max_iterations && !force_abort ) {
      ...
      bk_ti.start();
#endif
      run_synchronous( &powerlyra_sync_ec_engine::exchange_messages );
#ifdef TUNING
      ...
      bk_ti.start();
#endif
      run_synchronous( &powerlyra_sync_ec_engine::receive_messages );
      ...
      bk_ti.start();
#endif
      run_synchronous( &powerlyra_sync_ec_engine::execute_gathers );
      ...
      // NOTE: Third step is disabled since replicas do not need to get 
      //        updates in edge-cut model. All the source vertices are local
      //        masters, therefore gather can access the most recent data
      // if (rmi.procid() == 0) std::cout << "Applying..." << std::endl;
#ifdef TUNING
      bk_ti.start();
#endif
      run_synchronous( &powerlyra_sync_ec_engine::execute_applys );
#ifdef TUNING
       ...
       *      with all mirrors. NOTE: Not for edge-cut mode. It has been 
       *      disabled on purpose.
      ...
      bk_ti.start();
#endif
      run_synchronous( &powerlyra_sync_ec_engine::execute_scatters );
#ifdef TUNING
  ...
  template<typename VertexProgram>
  inline bool powerlyra_sync_ec_engine<VertexProgram>::
  ...
  execute_applys(const size_t thread_id) {
    ...
    while (1) {
      ...
      foreach(size_t lvid_block_offset, local_bitset) {
        ...
        // NOTE: by @anilpacaci both updates are disabled since mirrors do not
        // have to get most most recent data in edge-cut model. All edges are 
        // grouped by source vertex, therefore all source vertices are local
        // master. So gathers only access masters
  ...
  // Data Synchronization ===================================================
  template<typename VertexProgram>
  inline void powerlyra_sync_ec_engine<VertexProgram>::
