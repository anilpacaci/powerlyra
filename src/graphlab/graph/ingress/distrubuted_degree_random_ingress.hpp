...
namespace graphlab {
  ...
  public:
    ...
    void add_edge(vertex_id_type source, vertex_id_type target,
      ...
      procid_t procid;
      if(degree_vector.at(source) > degree_vector.at(target)) procid = graph_hash::hash_vertex(source) % base_type::rpc.numprocs();
      else graph_hash::hash_vertex(target) % base_type::rpc.numprocs();
